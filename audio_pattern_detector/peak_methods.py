...
class PeakProfile(TypedDict):
    """Profile of a peak in a data series."""
    width_100: int
    width_75: float | NDArray[np.float64]
    width_50: float | NDArray[np.float64]
    prominence: float
    left_trough: int
    right_trough: int


def find_closest_troughs(peak_index: int, data: NDArray[np.float32]) -> tuple[int, int]:
...
def calculate_peak_prominence(peak_index: int, data: NDArray[np.float32]) -> tuple[float, int, int]:
    ...
    Returns:
        A tuple of (prominence, left_trough_index, right_trough_index).
    """
    left_trough, right_trough = find_closest_troughs(peak_index, data)
    trough_height = max(data[left_trough], data[right_trough])
    prominence = float(data[peak_index] - trough_height)
    return prominence, left_trough, right_trough


def get_peak_profile(peak_index: int, data: NDArray[np.float32]) -> PeakProfile:
    """Get profile information for a peak."""
