...
| `--multiplexed-stdin`  | Read patterns and audio from stdin using binary protocol (for IPC)       | No              |
| `--raw-pcm`            | With `--stdin` or `--multiplexed-stdin`: read raw float32 PCM audio      | No              |
...
# Use 16kHz for AI workflows (helpful if source is already at 16kHz because pattern.wav will be resampled for matching at 16kHz if needed and source does not need to be resampled)
...
**This mode does not require ffmpeg** and does not require `--pattern-file` or `--pattern-folder` (patterns are sent via stdin).
